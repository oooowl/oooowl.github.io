...
// node_modules/.pnpm/vue-data-ui@2.4.1/node_modules/vue-data-ui/dist/vue-ui-timer-COSI3Q5T.js
var he = class {
  constructor(A, k, T, H = true, x2 = true) {
    this.interval = k, this.elapsed = 0, this.isPaused = false;
    const S = `
            let interval;
            let elapsed = 0;
            let paused = false;
...
var Se = {
  ...
  setup(G, { expose: A, emit: k }) {
    const T = G, { vue_ui_timer: H } = oe(), x2 = ref(null), S = ref(null), R = ref(null), m = ref(null), i = ref(Lt()), I = ref(0);
    onMounted(() => {
      c2();
    });
    function c2() {
    ...
    function L() {
      k("start"), D.value && u2.start(), D.value = false, s2.value = true;
    }
    function N() {
      s2.value && (k("reset"), u2.stop(), b.value = [], D.value = true, s2.value = false);
    }
    function B() {
      p2.value = !p2.value, k("pause", d.value), u2.pause();
    }
    function j() {
      s2.value && (p2.value = false, k("restart"), b.value = [], u2.restart());
    }
    const b = ref([]);
    async function F() {
...
//# sourceMappingURL=vue-ui-timer-COSI3Q5T-FC2FWCPH.js.map
